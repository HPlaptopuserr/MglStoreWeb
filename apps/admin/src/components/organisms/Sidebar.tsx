...
export function Sidebar({
  ...
  const defaultNavItems: NavItem[] = [
    ...
    { id: "partners", label: "Түншүүд", icon: Users, href: "/partners" },
    {
      id: "sections",
  ...
  return (
    <aside
      className={`${isCollapsed ? "w-22" : "w-65"} bg-white border-r border-slate-100 flex flex-col pt-8 pb-6 px-4 shrink-0 shadow-[2px_0_8px_rgba(0,0,0,0.02)] transition-all duration-300 relative`}
    >
      <button
      ...
      <div
        className={`flex items-center gap-3 mb-10 ${isCollapsed ? "justify-center px-0" : "px-4"}`}
      >
        <div className="bg-[#5B4CFF] text-white p-2 rounded-xl shrink-0">
          <ShieldCheck className="w-5 h-5" />
        </div>
        {!isCollapsed && (
          <span className="text-xl font-bold text-slate-800 whitespace-nowrap overflow-hidden transition-opacity duration-300">
            Marrow
          </span>
        )}
      </div>
      ...
      <div className="mt-auto">
        <div
          className={`bg-[#F8F9FE] rounded-2xl flex flex-col gap-4 transition-all duration-300 ${isCollapsed ? "p-2" : "p-4"}`}
        >
          <div
          ...
          >
            <LogOut
              className={`w-4 h-4 shrink-0 ${isCollapsed ? "" : "ml-1"}`}
            />
            {!isCollapsed && (
