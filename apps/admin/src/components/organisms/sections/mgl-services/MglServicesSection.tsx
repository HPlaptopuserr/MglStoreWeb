...
function compressImage(file: File): Promise<Blob> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        (blob) => (blob ? resolve(blob) : reject(new Error("Compress алдаа"))),
        "image/jpeg",
        0.85,
      );
    };
...
interface Props {
  ...
  title?: string;
  description?: string;
  defaultCategory?: Partial<ServiceCategory>;
}

export function MglServicesSection({
  mglServices,
  setMglServices,
  onSave,
  saving,
  saved,
  title = "MGL Үйлчилгээнүүд",
  description = "MGL Store-оос санал болгож буй багц үйлчилгээнүүдийн тохиргоо",
  defaultCategory,
}: Props) {
  ...
  const addCategory = () => {
    const defaultId =
      defaultCategory?.id &&
      !mglServices.some((category) => category.id === defaultCategory.id)
        ? defaultCategory.id
        : generateId();
    setMglServices([
      ...mglServices,
      {
        id: defaultId,
        title: defaultCategory?.title || "Шинэ ангилал",
  ...
  const updateCategory = (
    ...
    setMglServices(
      mglServices.map((c) => (c.id === id ? { ...c, [field]: value } : c)),
    );
  };
  ...
  const updateSubCategory = (
    catId: string,
    subId: string,
    field: keyof ServiceSubCategory,
    value: any,
  ) => {
    setMglServices(
      mglServices.map((c) => {
            ...
            subCategories: c.subCategories.map((sub) => {
              if (sub.id === subId) {
                return {
                  ...
                  items: [
                    ...sub.items,
                    { id: generateId(), name: "Шинэ үйлчилгээ", price: 0 },
                  ],
                };
              }
              return sub;
                  ...
                  items: sub.items.map((item) => {
                    if (item.id === itemId) {
                      return {
                        ...
                        options: [
                          ...(item.options || []),
                          { id: generateId(), name: "Шинэ сонголт", price: 0 },
                        ],
                      };
                    }
                    return item;
            ...
            >
              <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => moveCategory(mglServices.indexOf(cat), "up")}
                  ...
                  <button
                    onClick={() =>
                      moveCategory(mglServices.indexOf(cat), "down")
                    }
                    className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-200 rounded-md transition-colors"
                    title="Доошлуулах"
                  >
                  ...
                  <button
                    ...
                    className="text-red-400 hover:text-red-600 p-1.5 rounded-md hover:bg-red-50 ml-2"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="space-y-4 pt-2">
                    {cat.subCategories.map((sub) => (
                      <div
                        key={sub.id}
                        className="border border-slate-100 rounded-xl bg-slate-50/50 p-4"
                      >
                        ...
                        <div className="space-y-2">
                          {sub.items.map((item) => (
                            <div
                              key={item.id}
                              className="bg-white border border-slate-200 rounded-lg p-3 flex flex-col gap-2 relative group"
                            >
                              <button
                                onClick={() =>
                                  removeItem(cat.id, sub.id, item.id)
                                }
                              ...
                              <div className="mt-2 border-t border-slate-100 pt-3">
                                <div className="flex items-center justify-between mb-2">
                                  <label className="text-[10px] uppercase font-bold text-slate-400">
                                    Нэмэлт сонголтууд (Сонгох боломжтой үнэ)
                                  </label>
                                  <button
                                    onClick={() =>
                                      addOption(cat.id, sub.id, item.id)
                                    }
                                    className="text-xs text-blue-500 hover:text-blue-600 flex items-center gap-1 font-medium bg-blue-50 px-2 py-1 rounded"
                                  >
                                ...
                                {item.options && item.options.length > 0 && (
                                  <div className="space-y-2 mt-2">
                                    {item.options.map((opt) => (
                                      <div
                                        key={opt.id}
                                        className="flex gap-3 items-center bg-white p-2 rounded-xl border border-slate-200 group/opt hover:border-blue-200 hover:shadow-sm transition-all"
                                      >
                                        ...
                                        <button
                                          ...
                                          className="text-slate-300 hover:text-red-500 hover:bg-red-50 p-1.5 opacity-0 group-hover/opt:opacity-100 transition-all rounded-md mr-1"
                                        >
                                          <Trash2 size={14} />
                                        </button>
                                      </div>
                          ...
                          <button
                            ...
                            className="w-full border-2 border-dashed border-slate-200 rounded-lg py-2 flex items-center justify-center gap-1.5 text-xs font-medium text-slate-500 hover:border-slate-300 hover:bg-slate-50 transition-colors"
                          >
      ...
      {mglServices.length === 0 && (
        <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-2xl">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Та "Үндсэн ангилал нэмэх" товчийг дарж шинээр үүсгэнэ үү.
          </p>
        </div>
      )}
    </div>
