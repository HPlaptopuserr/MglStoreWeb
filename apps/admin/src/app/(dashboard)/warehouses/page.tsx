...
export default function AdminWarehousesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (warehouseRes.ok) {
        const data: unknown = await warehouseRes.json();
        const warehouseList = Array.isArray(data)
          ? (data as WarehouseData[])
          : [];

        // Vendor inventory warehouses are implementation details and must never
        // appear in the independently managed warehouse administration screen.
        setWarehouses(
          warehouseList.filter(
            (warehouse) =>
              warehouse.type !== "VENDOR_INTERNAL" &&
              !warehouse.name.trim().endsWith("- Үндсэн агуулах"),
          ),
        );
      }
                    ...
                    {filteredOrgs.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-10 text-center">
                        ...
                        <p className="text-xs text-slate-400 mt-1">
                          &ldquo;{orgSearchTerm}&rdquo; хайлтад тохирох vendor
                          байхгүй
                        </p>
                        <button
                          onClick={() => setOrgSearchTerm("")}
                          ...
                          return (
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setSelectedOrgIds(
                                    selectedOrgIds.filter((sid) => sid !== id),
                                  )
                                }
