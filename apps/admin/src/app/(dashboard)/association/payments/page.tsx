...
import { ArrowLeft, Banknote, Loader2, RefreshCw, Search, X } from "lucide-react";
...
export default function AssociationPaymentsPage() {
  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      const res = await adminFetch(`${API}/admin/association/registrations?limit=300`);
      if (res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900">Гишүүнчлэлийн төлбөр тооцоо</h1>
            <p className="text-xs text-slate-400">Холбооны төлбөртэй гишүүнчлэлийн шилжүүлэг бүртгэх</p>
          </div>
        </div>
        <button
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <SummaryCard label="Төлсөн дүн" value={`${paidTotal.toLocaleString()}₮`} tone="emerald" />
        <SummaryCard label="Хүлээгдэж буй дүн" value={`${pendingTotal.toLocaleString()}₮`} tone="amber" />
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center gap-3 py-24">
          ...
          <p className="text-sm font-semibold text-slate-400">Ачааллаж байна...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 bg-white py-20 text-center">
          <Banknote size={40} className="mx-auto mb-3 text-slate-300" />
          <p className="text-sm font-black text-slate-500">Төлбөртэй бүртгэл олдсонгүй</p>
        </div>
      ) : (
