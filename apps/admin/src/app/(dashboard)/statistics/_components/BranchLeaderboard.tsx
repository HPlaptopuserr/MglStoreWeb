...
import { money, windowLabel } from "./statistics-format";
...
export function BranchLeaderboard({
  ...
  const periodLabel = windowLabel(windowDays);
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        ...
        <span className="w-fit shrink-0 rounded-full bg-sky-50 px-3 py-1.5 text-xs font-black text-sky-700 ring-1 ring-sky-100">
          {periodLabel}
        </span>
      </div>
      <div className="space-y-3">
        ...
        {branches.slice(0, visibleCount).map((item, index) => {
          const barWidth = Math.min(
            100,
            Math.max(8, (item.revenue / Math.max(maxRevenue, 1)) * 100),
          );
          return (
            <article
              key={item.branchId}
            ...
            >
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-3 flex items-start gap-2 text-xs font-semibold leading-5 text-slate-500">
                    ...
                    <span className="line-clamp-2">
                      {item.address || "Хаяг бүртгэгдээгүй"}
                    </span>
                  </p>
                </div>
                <div className="shrink-0 text-left lg:text-right">
                  ...
                  <p className="text-xs font-black text-emerald-600">
                    Нийт орлогын {item.sharePercent}%
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-2 text-xs font-bold text-slate-500 sm:grid-cols-4">
                <Metric
                  ...
                  value={item.orders.toLocaleString("mn-MN")}
                />
                <Metric
                  label="POS"
                  value={item.posSales.toLocaleString("mn-MN")}
                />
                <Metric
                  label="Online"
                  value={item.onlineOrders.toLocaleString("mn-MN")}
                />
