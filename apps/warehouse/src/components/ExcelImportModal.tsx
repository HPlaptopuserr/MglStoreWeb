...
export function ExcelImportModal({
  ...
  const downloadTemplate = async () => {
    try {
      const res = await wmsFetch(`${API}/products/import-template`);
...
function ImportResults({
  ...
  const uploadToServer = async (file: File): Promise<string | null> => {
    ...
    try {
      const res = await wmsFetch(`${API}/products/upload-image`, {
        method: "POST",
        body: formData,
      });
  ...
  const saveImages = async (productId: string) => {
    ...
    try {
      const res = await wmsFetch(`${API}/products/${productId}/images`, {
        method: "PATCH",
