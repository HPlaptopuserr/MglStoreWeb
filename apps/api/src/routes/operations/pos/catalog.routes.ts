    ...
    const response = products
      .map((p) => {
        ...
        return {
          ...
          isTakeawayAvailable: p.isTakeawayAvailable,
          takeawayPackagingFee: Number(p.takeawayPackagingFee || 0),
          isSoldByPiece: p.isSoldByPiece,
          pieceSmallPackSize: p.pieceSmallPackSize,
          pieceSmallPackFee: Number(p.pieceSmallPackFee || 0),
          pieceLargePackSize: p.pieceLargePackSize,
          pieceLargePackFee: Number(p.pieceLargePackFee || 0),
...
router.get("/pos/sales/history", async (req, res) => {
  try {
    ...
    const result = sales.map((sale) => ({
      ...
      branchName: sale.branch.name,
      registerId: sale.register?.id || null,
