...
function getContextIdentity(context?: QPayMerchantContext): {
  ...
  const cacheKey =
    context?.merchantKey ||
    `default:${username}:${terminalId || "no-terminal"}`;
...
async function fetchNewToken(
  ...
  console.log(
    `[QPay] Token fetched successfully, expires in ${data.expires_in}s`,
  );
  return {
...
async function getAccessToken(context?: QPayMerchantContext): Promise<string> {
  ...
  const { cacheKey, username, password, terminalId } =
...
export async function createQPayInvoice(params: {
  ...
  if (!publicUrl || publicUrl === "https://api.mglstore.mn") {
    console.warn(
      "[QPay] API_PUBLIC_URL may not be configured correctly:",
      publicUrl,
    );
  }

  const callbackPath =
    params.callbackConfig?.path || "/api/store/qpay/callback";
  const callbackQuery = new URLSearchParams({ orderId: params.orderId });

  for (const [key, value] of Object.entries(
    params.callbackConfig?.query || {},
  )) {
  ...
  const quickQrContext = isQuickQrContext(params.merchantContext)
  ...
  if (quickQrContext) {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `QuickQR create invoice failed: ${res.status} - ${errBody}`,
      );
    }
  ...
  if (!invoiceCode) {
    ...
    throw new Error(
      "QPay invoice code (QPAY_INVOICE_CODE) is not configured in environment variables",
    );
  }

  // Validate the effective credentials. A server-owned flow such as Pro
  // Upgrade can intentionally supply its own merchant context instead of the
  // shared QPAY_CLIENT_ID/QPAY_CLIENT_SECRET values.
  getContextIdentity(params.merchantContext);
  ...
  const body = {
    ...
    invoice_description:
      params.description || `MGL Store - ${params.orderNumber}`,
    amount: params.amount,
    callback_url: callbackUrl,
  };
...
export async function registerQPayMerchantCompany(
  ...
  console.log(
    "QPay company registration raw:",
    JSON.stringify(redactQPayRegistrationResponse(raw)),
  );
  return {
    raw,
...
export async function getQPayCityList(): Promise<
...
export async function getQPayDistrictList(
  cityCode: string,
...
export async function checkQPayPayment(
  ...
  if (isQuickQr && raw.payments) {
    ...
    const paidPayments = payments.filter(
      (p) =>
        String(p.payment_status || p.status || "").toUpperCase() === "SUCCESS",
    );
    return {
      count: paidPayments.length,
      paid_amount: paidPayments.reduce(
