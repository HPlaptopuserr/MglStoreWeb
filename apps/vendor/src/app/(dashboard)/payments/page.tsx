...
export default function PaymentsPage() {
  const [user, setUser] = useState<{ id: string; organizationId?: string } | null>(null);
  ...
  // Computed
  const stats = useMemo(() => {
    const unpaid = payments.filter((p) => p.status === "PENDING" || p.status === "FAILED");
    const paid = payments.filter((p) => p.status === "PAID");
    const overdue = unpaid.filter((p) => p.dueDate && new Date(p.dueDate) < new Date());
    return {
      unpaidCount: unpaid.length,
      paidCount: paid.length,
      overdueCount: overdue.length,
      totalUnpaid: unpaid.reduce((s, p) => s + (Number(p.totalAmount) - Number(p.paidAmount)), 0),
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      if (sortOrder === "amount") return Number(b.totalAmount) - Number(a.totalAmount);
  ...
  const submitPayment = async () => {
    ...
    if (!paymentMethod) { setPayError("Төлбөрийн хэлбэр сонгоно уу"); return; }
    setPaying(true);
    setPayError("");
    try {
      const res = await authFetch(`${API}/stock-requests/payments/${payingPayment.id}/pay`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ paymentMethod, transactionId: transactionId.trim() || null, note: payNote.trim() || null }),
      });
      if (!res.ok) {
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div className="group relative overflow-hidden rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50 to-orange-50 p-5 transition-shadow hover:shadow-lg hover:shadow-amber-100/50">
            ...
            <div className="relative">
              ...
              <p className="text-3xl font-black text-amber-900">{stats.unpaidCount}</p>
              <p className="mt-0.5 text-sm font-medium text-amber-600/80">Төлөгдөөгүй</p>
            </div>
          </div>
          ...
          <div className="group relative overflow-hidden rounded-2xl border border-red-100 bg-gradient-to-br from-red-50 to-rose-50 p-5 transition-shadow hover:shadow-lg hover:shadow-red-100/50">
            ...
            <div className="relative">
              ...
              <p className="mt-0.5 text-sm font-medium text-red-600/80">Төлөх дүн</p>
            </div>
          </div>
          ...
          <div className="group relative overflow-hidden rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-green-50 p-5 transition-shadow hover:shadow-lg hover:shadow-emerald-100/50">
            ...
            <div className="relative">
              ...
              <p className="text-3xl font-black text-emerald-900">{stats.paidCount}</p>
              <p className="mt-0.5 text-sm font-medium text-emerald-600/80">Төлөгдсөн</p>
            </div>
          </div>
          ...
          <div className="group relative overflow-hidden rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-indigo-50 p-5 transition-shadow hover:shadow-lg hover:shadow-blue-100/50">
            ...
            <div className="relative">
              ...
              <p className="mt-0.5 text-sm font-medium text-blue-600/80">Нийт төлсөн</p>
            </div>
          </div>
        </div>
          ...
          <div className="mb-6 flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-5">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-red-700">
                Хугацаа хэтэрсэн нэхэмжлэх байгаа тул шинэ хүсэлт зөвшөөрөгдөхгүй. Нэн даруй төлнө үү.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {(["ALL", "PENDING", "PAID", "FAILED", "CANCELLED"] as StatusFilter[]).map((status) => {
              const count =
                status === "ALL"
                  ? payments.length
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() =>
                setSortOrder((prev) =>
                  prev === "newest" ? "oldest" : prev === "oldest" ? "amount" : "newest"
                )
              }
              ...
              title={sortOrder === "newest" ? "Шинэ → Хуучин" : sortOrder === "oldest" ? "Хуучин → Шинэ" : "Дүнгээр"}
            >
              <ArrowUpDown className="h-4 w-4" />
              <span className="hidden sm:inline">
                {sortOrder === "newest" ? "Шинэ" : sortOrder === "oldest" ? "Хуучин" : "Дүн"}
              </span>
            </button>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 bg-white py-20">
            ...
            <p className="text-xl font-bold text-slate-600">
              {searchQuery || statusFilter !== "ALL" ? "Хайлтад тохирох нэхэмжлэх олдсонгүй" : "Төлбөрийн түүх байхгүй"}
            </p>
          ...
          <div className="space-y-3">
            {filtered.map((payment) => {
              const isUnpaid = payment.status === "PENDING" || payment.status === "FAILED";
              const isOverdue = isUnpaid && payment.dueDate && new Date(payment.dueDate) < new Date();
              const sc = STATUS_COLORS[payment.status];
              const daysLeft = payment.dueDate ? daysUntilDue(payment.dueDate) : null;
                    ...
                    <div className="flex items-center gap-3 sm:flex-col sm:items-end sm:gap-2">
                      ...
                      {isUnpaid ? (
                        ...
                        <p className="text-xs text-emerald-600">
                          {new Date(payment.paidAt).toLocaleDateString("mn-MN")} төлсөн
                        </p>
                      ) : null}
                      <button
                        onClick={() => openDetail(payment.id)}
              ...
              {loadingDetail ? (
                ...
                <>
                  ...
                  <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white/95 px-6 py-4 backdrop-blur-sm print:hidden">
                    <div>
                      <h2 className="text-lg font-bold text-slate-900">Нэхэмжлэх</h2>
                      <p className="text-sm text-slate-500">{selectedPayment.invoiceNumber}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => { setShowDetail(false); setSelectedPayment(null); }}
                  ...
                  <div className="p-6 print:p-4">
                    {/* Status banner */}
                    <div className={`mb-6 rounded-2xl p-4 ${STATUS_COLORS[selectedPayment.status].bg}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <span className={`h-3 w-3 rounded-full ${STATUS_COLORS[selectedPayment.status].dot}`} />
                          <span className={`text-lg font-bold ${STATUS_COLORS[selectedPayment.status].text}`}>
                    ...
                    <div className="mb-6 grid grid-cols-2 gap-4 text-sm">
                      <div className="rounded-xl bg-slate-50 p-3">
                        ...
                        <p className="mt-0.5 font-bold text-slate-900">{selectedPayment.invoiceNumber}</p>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-3">
                        <p className="text-slate-400">Огноо</p>
                        <p className="mt-0.5 font-bold text-slate-900">
                          {new Date(selectedPayment.createdAt).toLocaleDateString("mn-MN")}
                        </p>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-3">
                      ...
                      {selectedPayment.dueDate && (
                        <div className="rounded-xl bg-slate-50 p-3">
                          ...
                          <p className={`mt-0.5 font-bold ${
                            new Date(selectedPayment.dueDate) < new Date() && selectedPayment.status === "PENDING"
                              ? "text-red-600" : "text-slate-900"
                          }`}>
                            {new Date(selectedPayment.dueDate).toLocaleDateString("mn-MN")}
                          </p>
                        </div>
                      )}
                      {selectedPayment.paidAt && (
                        <div className="rounded-xl bg-emerald-50 p-3">
                          ...
                          <p className="mt-0.5 font-bold text-emerald-700">
                            {new Date(selectedPayment.paidAt).toLocaleDateString("mn-MN")}
                          </p>
                        </div>
                      )}
                      {selectedPayment.paymentMethod && (
                        <div className="rounded-xl bg-slate-50 p-3">
                          ...
                          <p className="mt-0.5 font-bold text-slate-900">
                            {selectedPayment.paymentMethod === "CASH" ? "Бэлнээр" :
                             selectedPayment.paymentMethod === "BANK" ? "Банк шилжүүлэг" :
                             selectedPayment.paymentMethod === "QPAY" ? "QPay" :
                             selectedPayment.paymentMethod}
                          </p>
                        </div>
                      )}
                        ...
                        <table className="w-full text-sm">
                          <thead className="bg-slate-50">
                            <tr>
                              ...
                              <th className="px-4 py-3 text-left font-semibold text-slate-500">Бараа</th>
                              <th className="px-4 py-3 text-center font-semibold text-slate-500">Тоо</th>
                              <th className="px-4 py-3 text-right font-semibold text-slate-500">Үнэ</th>
                              <th className="px-4 py-3 text-right font-semibold text-slate-500">Нийт</th>
                            </tr>
                          </thead>
                ...
                <div className="p-8 text-center text-slate-500">Нэхэмжлэх олдсонгүй</div>
              )}
            </div>
          </div>
        )}
            ...
            <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl">
              {paySuccess ? (
                <div className="flex flex-col items-center p-10">
                  ...
                  <h2 className="text-xl font-bold text-slate-900">Амжилттай төлөгдлөө!</h2>
                ...
                <>
                  ...
                  <div className="border-b border-slate-100 px-6 py-5">
                    <div className="flex items-center justify-between">
                      <div>
                        <h2 className="text-lg font-bold text-slate-900">Төлбөр хийх</h2>
                        <p className="text-sm text-slate-500">{payingPayment.invoiceNumber}</p>
                      </div>
                      <button
                    ...
                    <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-center">
                      ...
                      <p className="text-4xl font-black text-slate-900">
                        {(Number(payingPayment.totalAmount) - Number(payingPayment.paidAmount)).toLocaleString()}₮
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-5 p-6">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {[
                          { value: "BANK", label: "Банк шилжүүлэг", icon: Banknote, color: "blue" },
                          { value: "QPAY", label: "QPay", icon: Smartphone, color: "violet" },
                          { value: "CASH", label: "Бэлнээр", icon: DollarSign, color: "emerald" },
                        ].map((method) => {
                            ...
                            >
                              ...
                              <span
                                className={`text-xs font-semibold ${
                                  isSelected ? `text-${method.color}-700` : "text-slate-500"
                                }`}
                              >
                                {method.label}
                              </span>
                    ...
                    {/* Transaction ID */}
                    {(paymentMethod === "BANK" || paymentMethod === "QPAY") && (
                      <div>
