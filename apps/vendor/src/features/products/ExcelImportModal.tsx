...
const isExcelFile = (file: File) => {
  ...
  return (
    ...
    EXCEL_FILE_EXTENSIONS.some((extension) => fileName.endsWith(extension))
  );
};
    ...
    : [
        {
          ...
          req: false,
          desc: "Хоосон бол 14 хоног гэж авна",
        },
        {
          col: "Захиалгын тайлбар (preorderNote)",
          req: false,
...
export function ExcelImportModal({
  ...
  const downloadTemplate = async () => {
    try {
      ...
      const res = await authFetch(
        `${API}/products/import-template?${params.toString()}`,
      );
  ...
  const handleFile = (file: File) => {
    ...
    if (file.size > MAX_EXCEL_FILE_SIZE_BYTES) {
      setError(`Файлын хэмжээ ${MAX_EXCEL_FILE_SIZE_MB}MB-аас хэтэрсэн байна`);
      return;
    }
    setError(null);
      ...
      <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-top-4">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center ${copy.accentClass}`}
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      {copy.subtitle}. Excel-н бүтэц, баганы нэрсийг харж
                      бөглөнө үү
                    </p>
                  </div>
                </div>
                ...
                {selectedFile ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      эсвэл файл сонгоно уу (.xlsx, .xls · дээд хэмжээ{" "}
                      {MAX_EXCEL_FILE_SIZE_MB}MB)
                    </p>
                    <button
...
function ImportResults({
  ...
  const [editableRows, setEditableRows] = useState<ImportErrorRow[]>(
    result.errorRows || [],
  );
  ...
  const productIdsWithImages = Object.keys(productImages).filter(
    (id) => productImages[id]?.length > 0,
  );

  const saveAllImages = async () => {
    for (const id of productIdsWithImages) {
  ...
  const allSaved = productIdsWithImages.every((id) => savedImages.has(id));
  const canRetryEditedRows = editableRows.some(
          ...
          <div className="px-4 py-3 bg-blue-50 border-b border-blue-100 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[11px] text-blue-600 mt-0.5">
                SKU давхардсан бол SKU-г өөр код болгож засна. Нэр, үнэ хоосон
                байж болохгүй.
              </p>
            </div>
            <button
                  ...
                  <tr key={`${row.rowNumber}-${index}`} className="text-xs">
                    ...
                    <td className="px-3 py-2">
                      <input
                        value={row.name}
                        onChange={(e) =>
                          updateEditableRow(index, "name", e.target.value)
                        }
                    ...
                    <td className="px-3 py-2">
                      <input
                        value={row.costPrice}
                        onChange={(e) =>
                          updateEditableRow(index, "costPrice", e.target.value)
                        }
                        className="h-9 w-full rounded-lg border border-slate-200 px-2 text-xs text-slate-700 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                      />
                    </td>
