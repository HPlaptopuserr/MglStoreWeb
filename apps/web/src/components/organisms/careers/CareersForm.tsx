...
export function CareersForm() {
  ...
  useEffect(() => {
    const fetchJobPositions = async () => {
      try {
        setLoadingJobs(true);
        const res = await fetch(`${API}/job-positions`, { cache: "no-store" });
        if (!res.ok) throw new Error("Ажлын байр авахад алдаа гарлаа");
  ...
  return (
    <div className="bg-white rounded-4xl shadow-2xl p-8 md:p-10 border border-white/20 backdrop-blur-sm min-h-150 flex flex-col">
      <div className="flex items-center justify-between mb-8 border-b border-gray-100 pb-6">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">Анкет бөглөх</h3>
          <div className="flex items-center gap-2 mt-2">
            {[1, 2, 3].map((step) => (
              <div
                key={step}
                className={`h-2 w-8 rounded-full transition-colors ${
                  step <= currentStep ? "bg-[#FFB700]" : "bg-gray-200"
                }`}
              />
            ))}
        ...
        {submitted ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
            ...
            <p className="text-gray-500 max-w-sm">
              Таны анкет амжилттай хүлээн авлаа. Бид тантай удахгүй холбогдох
              болно.
            </p>
          </div>
        ) : (
