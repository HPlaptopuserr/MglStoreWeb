  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {onDeleteAccount && (
        <section
          id="danger-zone"
          className="rounded-[20px] border border-red-200 bg-red-50/50 p-3.5 shadow-sm md:rounded-3xl md:p-6"
        >
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-red-600 md:text-xs md:tracking-[0.22em]">
                Danger Zone
              </p>
              <h3 className="mt-1 text-lg font-black text-red-950 md:text-xl">
                Бүртгэл устгах
              </h3>
              <p className="mt-1 text-xs font-semibold text-red-800/80 md:text-sm">
                Бүртгэлээ устгаснаар таны бүх хувийн мэдээлэл, хаяг болон хандах эрхүүд
                системээс бүрмөсөн устгагдах болно.
              </p>
            </div>
            <button
              type="button"
              ...
              className="inline-flex shrink-0 items-center justify-center gap-2 rounded-2xl bg-red-600 px-5 py-3.5 text-sm font-bold text-white shadow-sm transition hover:bg-red-700 disabled:opacity-60"
            >
              {deletingAccount ? <Loader2 size={16} className="animate-spin" /> : <AlertCircle size={16} />}
              {deletingAccount ? "Устгаж байна..." : "Бүртгэл устгах"}
            </button>
          </div>
        </section>
      )}
