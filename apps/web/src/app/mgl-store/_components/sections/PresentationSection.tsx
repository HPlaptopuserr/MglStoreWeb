  ...
  return (
    <section id="presentation" className="scroll-mt-28 bg-white py-14 sm:py-20">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10 xl:px-16">
        <SectionHeading eyebrow="Дэлгэрэнгүй танилцуулга" title="MGL STORE" description="" />
        <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-6">
          <div className="min-w-0 max-w-full overflow-hidden rounded-[1.5rem] bg-slate-950 shadow-2xl shadow-slate-300/60 sm:rounded-[2rem]">
            ...
            <div className="min-w-0 bg-slate-100 p-1.5 sm:p-3"><Image key={activeSlide.src} src={activeSlide.src} alt={activeSlide.title} width={1280} height={720} className="aspect-video h-auto max-w-full animate-[slideFade_.35s_ease-out] rounded-[1rem] object-contain sm:rounded-[1.35rem]" sizes="(min-width: 1280px) 950px, 100vw" priority={activeSlideIndex === 0} /></div>
          </div>
          <aside aria-label="Танилцуулгын слайдууд" className="min-w-0 max-w-full rounded-[1.5rem] border border-slate-200 bg-slate-50 p-3 sm:p-4 lg:rounded-[2rem]">
