...
function matchesDiscoveryKeyword(
  text: string,
  tokens: string[],
  keyword: string,
) {
...
function isBroadIntentSearch(
  search: string,
  intent: (typeof SEARCH_INTENTS)[number],
) {
  const normalizedSearch = normalizeDiscoveryText(search);
  ...
  const intentPhrases = [intent.label, ...intent.aliases].map(
    normalizeDiscoveryText,
  );
  return intentPhrases.some(
    (phrase) => phrase && normalizedSearch.includes(phrase),
  );
}
...
export function createProductSearchScorer(search: string) {
  const normalizedSearch = normalizeDiscoveryText(search);
  if (!normalizedSearch) return (_product: SearchableProduct) => 0;

  const tokens = tokenizeDiscoveryText(search);
  const searchVariants = [
    ...new Set(expandSearchPhraseVariants(search).map(normalizeDiscoveryText)),
  ];
  const tokenVariants = [
    ...new Set(
      tokens
        .flatMap((token) => [
          token,
          ...transliterateLatinTokenToCyrillic(token),
        ])
        .map(normalizeDiscoveryText),
    ),
  ];
  return (product: SearchableProduct) => {
    const fields = {
      ...
      taxProductCode: normalizeDiscoveryText(product.taxProductCode || ""),
      category: normalizeDiscoveryText(
        `${product.categoryName || ""} ${product.businessCategory?.name || ""} ${product.businessCategory?.slug || ""} ${product.businessCategory?.parent?.name || ""} ${product.businessCategory?.parent?.slug || ""}`,
      ),
    ...
    for (const variant of searchVariants) {
      ...
      if (fields.sku.includes(variant) || fields.barcode.includes(variant))
        score += 22;
      if (
        fields.classificationCode.includes(variant) ||
        fields.taxProductCode.includes(variant)
      )
        score += 22;
    }
    if (fields.sku === normalizedSearch || fields.barcode === normalizedSearch)
      score += 100;
    ...
    for (const token of tokenVariants) {
      ...
      if (fields.sku.includes(token) || fields.barcode.includes(token))
        score += 22;
    }
    ...
    for (const intent of SEARCH_INTENTS) {
      if (
        ...
        const intentText = normalizeDiscoveryText(
          `${intent.label} ${intent.aliases.join(" ")}`,
        );
        if (isBroadIntentSearch(search, intent)) {
          if (fields.category.includes(normalizeDiscoveryText(intent.label)))
            score += 45;
          if (
            intent.aliases.some((alias) =>
              fields.category.includes(normalizeDiscoveryText(alias)),
            )
          ) {
            score += 30;
          }
        ...
        if (
          tokens.some(
            (token) =>
              intentText.includes(token) && fields.name.includes(token),
          )
        ) {
          score += 12;
        }
...
export function scoreProductForSearch(
  ...
  return createProductSearchScorer(search)(product);
}
...
export function scoreProductSimilarity(
  source: SearchableProduct,
  candidate: SearchableProduct,
) {
  ...
  const sourceTokens = tokenizeDiscoveryText(sourceText).filter(
  ...
  let score = 0;
  if (
    source.businessCategoryId &&
    source.businessCategoryId === candidate.businessCategoryId
  ) {
    score += 180;
  } else if (
  ...
  if (
    ...
    source.organization.id === candidate.organization?.id
  ) {
    score += 20;
  }
  ...
  for (const token of sourceTokens) {
    ...
    score += candidateName.includes(token)
      ? 18
      : candidateCategory.includes(token)
        ? 14
        : 7;
  }
...
export function suggestProductCategory(
  ...
  return categories
    .map((category) => {
      const categoryText = normalizeDiscoveryText(
      ...
      for (const intent of SEARCH_INTENTS) {
        ...
        if (categoryText.includes(normalizeDiscoveryText(intent.label)))
          score += 50;
        if (
          intent.aliases.some((alias) =>
            categoryText.includes(normalizeDiscoveryText(alias)),
          )
        ) {
          score += 34;
        }
    ...
    .sort((a, b) => {
      ...
      const aLevel =
        categories.find((category) => category.id === a.id)?.level ?? 0;
      const bLevel =
        categories.find((category) => category.id === b.id)?.level ?? 0;
      return bLevel - aLevel;
    })
    .slice(0, 3);
}

export function getProductSearchPhrases(search: string) {
